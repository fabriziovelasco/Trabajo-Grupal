...
En este caso de clusterización utilizaremos otras variables,veremos la lista de variables y seleccionaremos las que usaremos:

```{r}
list(names(data))
```
En esta ocasión mantendremos la variable dependiente, Gasto del gobierno en educación (%PIB) y cambiaremos las variables independientes por la tasa de fertilidad adolescente (ARF)  y el desempleo total (UET):

```{r}
data4 = data[,c(1,5,6,8)]
```
...
str(data4)
```
Las variables ARF y UET están como tipo character así que debemos cambiarlas por numeric:

```{r}
data4$ARF=as.numeric(data4$ARF)
data4$UET=as.numeric(data4$UET)
```
AHora que ya está bien codificado hacemos un summary y un boxplot:

```{r}
summary(data4)
boxplot(data4[,-1])
```
Como el gráfico sale mal debemos transformar los datos:
...
```{r}
dataClus2=data4[,-1]
row.names(dataClus2)=data4$`Country Name`
```
